...
% Basic approach without dependency or composition. F-Algebra for deep embedding.
...
To state that the semantic domain that type i represents is part of a larger collection of types, we use the following type class
...
Following is a flexibly typed Brent-Kung parallel prefix circuit:

\begin{verbatim}
c2 :: Circuit circuit => circuit
...
Then we can use the above functions to get  different properties of c2, depending on its type. For example, \textit{gwidth (c1 :: Compose Width Depth)} and \textit{gdepth (c2 :: Compose Width Depth)} will give us its width and depth,  respectively. 
...
So far we've only talked about extensibility in one dimension: \textit{// to be continuted...}
...
An algebra that returns \textit{Compose Width WellSized} is simply:
\begin{verbatim}
cAlg2 = widthAlg <+> wsAlg
\end{verbatim}

Then we can evaluate circuit c1 defined earlier and get its width by:
\begin{verbatim}
gwidth (fold cAlg2 c1)
\end{verbatim}

And check whether it is well-formed or not by:
\begin{verbatim}
gwellSized (fold cAlg2 c1)
\end{verbatim}
